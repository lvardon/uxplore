...
static
char 	sdf_path[255], opened = 0, gpsav = 0, splat_name[10],
                                    dashes[80];

const char *splat_version = "2.x";
...
char *dec2dms(double decimal)
{
	...
	static char str[255];
	str[0] = 0;
	snprintf(str, 250, "%d%c %d\' %d\"", degrees * sign, 176, minutes, seconds);
	return (str);
}
...
void ReadPath(struct site source, struct site destination)
{
	...
	for (distance = 0.0, c = 0; (total_distance != 0.0 && distance <= total_distance && c < ARRAYSIZE); c++, distance = miles_per_sample * (double)c) {
		...
		if (azimuth == HALFPI && (beta > HALFPI + lat1))
			lon2 = lon1 + PI;
		else
		if (fabs(num / den) > 1.0)
				lon2 = lon1;
...
void WriteSDF_DAT(char *name, int indx)
{
	(void) indx;
	...
	fwrite((const void*)&dem[indx], 1,
	       sizeof(dem[0]) - sizeof(dem[0].mask) - sizeof(dem[0].signal),
	       fd);
	fclose(fd);
...
static
int LoadSDF_CACHE(char *name)
{
	/* This function reads preprocessed SPLAT Data Files (.cache)
	...
	int	x, y, indx, minlat, minlon, maxlat, maxlon; //, data, indx, minlat, minlon, maxlat, maxlon;
	char	free_page = 0, sdf_file[255],
...
int LoadSDF_SDF(char *name)
{
	...
	char	free_page = 0, line[20], sdf_file[255],
	        path_plus_name[255];
	FILE	*fd;
...
int LoadSDF_BZ(char *name)
{
	...
	int	x, y, data, indx, minlat, minlon, maxlat, maxlon;
	char	free_page = 0, sdf_file[255], path_plus_name[255], *string;
	FILE	*fd;
	BZFILE	*bzfd;
...
char LoadSDF(char *name)
{
	...
	/* Try to load a preprocessed CACHE files first. */

	return_value = LoadSDF_CACHE(name);

	/* If that fails, try loading an uncompressed SDF first. */
	...
	if ((max_lon - min_lon) <= 180.0) {
		for (y = 0; y <= width; y++)
			for (x = min_lat; x <= max_lat; x++) {
				...
				char str[256];
				if (ippd == 3600)
					snprintf(str, 19, "%d:%d:%d:%d-hd", x, x + 1, ymin, ymax);
				else
					snprintf(str, 16, "%d:%d:%d:%d", x, x + 1, ymin, ymax);
				LoadSDF(str);
			}
	} else {
		for (y = 0; y <= width; y++)
